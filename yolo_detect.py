...
def annotate_image_cv2(img: np.ndarray, boxes_xyxy, scores, classes, names):
    ...
    for xyxy, conf, cls in zip(boxes_xyxy, scores, classes):
        ...
        cv2.putText(out, label, (x1, y1 - 4), 0, 1, (0, 0, 0), thickness=2, lineType=cv2.LINE_AA)
    return out
...
def main():
    ...
    # --- MODIFICATION START ---
    # The original was: parser.add_argument('--save-annotated', action='store_true', help='Save annotated images to output/images.')
    # It is modified to make saving the default (default=True). Use --no-save-annotated to disable.
    ...
    # Optional: run model.val() on provided data yaml
    try:
        if Path(args.data).exists():
            print("[INFO] Running model.val() on provided data yaml (split='test') — this may take time.")
            val_res = model.val(data=args.data, split="test", imgsz=args.imgsz, batch=args.batch)
            print("[INFO] Validation result:")
            print(val_res)
    except Exception as e:
        print(f"[WARN] model.val() failed or not supported in this environment: {e}")
